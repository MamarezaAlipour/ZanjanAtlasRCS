...
def generate_all_years_injury_heatmap(excel_path, logo_path, shp_path):
    ...
    heat_data = [
        ...
        for _, row in df.iterrows() if float(row['تعداد کل مصدومین در حادثه /نفر']) > 0
    ]
    HeatMap(
        heat_data,
        min_opacity=0.3,
    ...
    map_zanjan.save("atlas_zanjan_Injury_HeatMap_AllYears.html")