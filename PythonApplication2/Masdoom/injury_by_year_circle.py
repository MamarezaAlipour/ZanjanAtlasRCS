...
def generate_yearly_injury_circle_maps(excel_path, logo_path, shp_path):
    ...
    for year in range(1393, 1404):
        ...
        total_injury_year = int(df_year["تعداد کل مصدومین در حادثه /نفر"].sum())
        count_html = f"""
        <style>
        @font-face {{
        ...
        <div class="titr-box" style="position: fixed; bottom: 20px; left: 20px; z-index:9999;
            ...
            مجموع کل مصدومین در همه حوادث سال {year}: {total_injury_year}
        </div>
        """
        ...
        map_zanjan.save(f"atlas_zanjan_Injury_Circle_{year}.html")