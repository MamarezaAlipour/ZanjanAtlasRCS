...
def generate_all_years_treatment_heatmap(excel_path, logo_path, shp_path):
    df = pd.read_excel(excel_path)
    df = df.dropna(subset=['عرض جغرافیایی(N)', 'طول جغرافیایی(E)', 'درمان سرپايي توسط جمعیت/نفر'])
    df['درمان سرپايي توسط جمعیت/نفر'] = pd.to_numeric(df['درمان سرپايي توسط جمعیت/نفر'], errors='coerce').fillna(0)
    ...
    heat_data = [
        [row['عرض جغرافیایی(N)'], row['طول جغرافیایی(E)'], float(row['درمان سرپايي توسط جمعیت/نفر'])]
        for _, row in df.iterrows() if float(row['درمان سرپايي توسط جمعیت/نفر']) > 0
    ]
    HeatMap(
        heat_data,
        min_opacity=0.3,
    ...
    total_treatment = int(df["درمان سرپايي توسط جمعیت/نفر"].sum())
    count_html = f"""
    <style>
    @font-face {{
        font-family: 'BTitr';
    ...
    <div class="titr-box" style="position: fixed; bottom: 20px; left: 20px; z-index:9999;
        ...
        مجموع درمان سرپایی توسط جمعیت از سال 1393 تا 1403: {total_treatment}
    </div>
    """
    map_zanjan.get_root().html.add_child(Element(count_html))
    add_paygah_markers(map_zanjan, "./Data/paygah.xlsx", "./Data/paygah_icon.png")
    map_zanjan.save("atlas_zanjan_Treatment_HeatMap_AllYears.html")