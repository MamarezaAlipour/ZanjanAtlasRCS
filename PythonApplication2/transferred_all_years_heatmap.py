...
def generate_all_years_transferred_heatmap(excel_path, logo_path, shp_path):
    ...
    heat_data = [
        [row['عرض جغرافیایی(N)'], row['طول جغرافیایی(E)'], float(row['مصدومين انتقالي توسط جمعیت/نفر'])]
        for _, row in df.iterrows() if float(row['مصدومين انتقالي توسط جمعیت/نفر']) > 0
    ]
    HeatMap(
        heat_data,
        min_opacity=0.3,
    ...
    <div class="titr-box" style="position: fixed; bottom: 20px; left: 20px; z-index:9999;
        ...
        مجموع مصدومین انتقالي توسط جمعیت از سال 1393 تا 1403: {total_transferred}
    </div>
    """
    map_zanjan.get_root().html.add_child(Element(count_html))

    map_zanjan.save("atlas_zanjan_Transferred_HeatMap_AllYears.html")