...
def generate_all_years_flood_heatmap(excel_path, logo_path, shp_path):
    df = pd.read_excel(excel_path)
    df_flood = df[df['نوع حادثه'] == 'سیل و آبگرفتگی']
    df_flood = df_flood.dropna(subset=['عرض جغرافیایی(N)', 'طول جغرافیایی(E)'])
    ...
    <div class="titr-box" style="position: fixed; bottom: 20px; left: 20px; z-index:9999;
        ...
        تعداد کل حوادث سیل و آبگرفتگی از سال 1393 تا 1403: {len(df_flood)}
    </div>
    """
    ...
    map_zanjan.save("atlas_zanjan_Flood_HeatMap_AllYears.html")