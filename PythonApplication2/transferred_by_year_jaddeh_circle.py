...
def generate_yearly_transferred_jaddeh_circle_maps(excel_path, logo_path, shp_path):
    ...
    for year in range(1393, 1404):
        ...
        <div class="titr-box" style="position: fixed; bottom: 20px; left: 20px; z-index:9999;
            ...
            مجموع مصدومین انتقالي توسط جمعیت (حوادث جاده‌ای) سال {year}: {total_transferred_year}
        </div>
        """
        map_zanjan.get_root().html.add_child(Element(count_html))
        add_paygah_markers(map_zanjan, "./paygah.xlsx", "./paygah_icon.png")
        map_zanjan.save(f"atlas_zanjan_Transferred_Jaddeh_Circle_{year}.html")